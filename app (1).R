...
# ============================================================
# 1. DATA LOADING AND CLEANING  (identical to Final_report.Rmd)
# ============================================================
...
# ============================================================
# 2. LOAD MODEL M3 POSTERIOR (deployed app NEVER fits JAGS live --
#    shinyapps.io has no JAGS installed and a 60s startup limit.
#    posterior_M3.rds MUST be fit locally first and included in
...
# ============================================================
# 2b. BUILD COUNTY GEOMETRY FOR THE INTERACTIVE MAP
#     (cached to rds so it only downloads once)
# ============================================================
...
server <- function(input, output, session) {
  ...
  # Reactive value to hold the currently "pinned" county (via click)
  ...
  # Redraw fill colour when the metric selector changes, without
  # rebuilding the whole map (keeps zoom/pan position)
  ...
  # Click a county on the map -> pin it in the side panel
  ...
  dic_result <- eventReactive(input$run_dic, {
    ...
    # cache it locally so next time this loads instantly without refitting
